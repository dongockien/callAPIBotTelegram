...
type GetProductsBySlugV1Param struct {
	Slug string `uri:"slug" binding:"required,min=3,max=5"`
}
	...
	for key := range params.ProductInfo {
		if _, err := uuid.Parse(key); err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{
				"errors": gin.H{
					"product_info": fmt.Sprintf("Key`%s` trong prodct_info không phải là UUID hợp lệ", key),
				},
			})
			return
		}
